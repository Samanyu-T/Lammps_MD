...
neb_image_folder = os.path.join(output_folder,'neb_screw_disloc')
...
lmp.commands_list(lmp_class.init_from_datafile('%s/screw_disloc_he.data' % os.path.join(output_folder,'Data_Files')))

lmp_class.cg_min(lmp)
pe_1 = lmp.get_thermo('pe')

print(pe_1)
filename = 'screw_disloc'
...
with open('screw_disloc.neb', 'w') as file:
    file.write(neb_txt)
